...
class GestureHandler extends EventEmitter {
    ...
    this._client.onmessage = (e) => {
      ...
      if (msg.type === 'data' && msg.data.length > 0) {
        ...
        for (; i < msg.data.length; i++) {
          ...
          else if(data.type='drawGesture'){
            this.emit('drawGesture', {type: data.type, data: data.data});
          }
        }
      }
    };
  ...
  addNewGesture(data, nameGesture){
    let message = { type: 'addNewGesture2d', data: data, name: nameGesture }
    this._client.send(JSON.stringify(message));
  }
  deleteGesture(nameGesture){
    let message = { type: 'deleteGesture2d', name: nameGesture }
    console.log(message)
    this._client.send(JSON.stringify(message));
  }
  drawGesture(nameGesture){
    let message = { type: 'drawGesture2d', name: nameGesture }
    this._client.send(JSON.stringify(message));
  }
  clearDataset(){
    let message = { type: 'clearDataset'}
    this._client.send(JSON.stringify(message));
  }
}
